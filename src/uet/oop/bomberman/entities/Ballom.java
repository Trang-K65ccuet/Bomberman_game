        ...
        if (this._isdead == false) {
            ...
            if (count < temp) {
                ...
                if (Map.ar[(y + 26) / Sprite.SCALED_SIZE][(x + 28) / Sprite.SCALED_SIZE] == ' '
                        && Map.ar[(y) / Sprite.SCALED_SIZE][(x + 28) / Sprite.SCALED_SIZE] == ' ') {
                    x += step;
                    int temp = y % Sprite.SCALED_SIZE;
                    if (temp >= 20) y = ((y / Sprite.SCALED_SIZE) + 1) * Sprite.SCALED_SIZE;
                    else if (temp <= 10) y = (y / Sprite.SCALED_SIZE) * Sprite.SCALED_SIZE;
                }
            } else {
                img = Sprite.movingSprite(Sprite.balloom_left1, Sprite.balloom_left2, Sprite.balloom_left3,
                        x, 60).getFxImage();
                if (Map.ar[(y + 26) / Sprite.SCALED_SIZE][(x - step) / Sprite.SCALED_SIZE] == ' ') {
                    x -= step;
                    int temp = y % Sprite.SCALED_SIZE;
                    if (temp >= 20) y = ((y / Sprite.SCALED_SIZE) + 1) * Sprite.SCALED_SIZE;
                    else if (temp <= 10) y = (y / Sprite.SCALED_SIZE) * Sprite.SCALED_SIZE;
